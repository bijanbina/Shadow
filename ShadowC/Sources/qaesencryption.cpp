...
//    alignedText.append(getPadding(rawText.size(), m_blocklen));
    ...
    case CFB: {
        QByteArray ret;
        ret.append(byteXor(alignedText.left(1), cipher(expandedKey, iv)));
        for(int i=0; i < alignedText.size(); i+= 1) {
            if (i+1 < alignedText.size())
                ret.append(byteXor(alignedText.mid(i+1, 1),
                                   cipher(expandedKey, ret.mid(i, 1))));
        }
        return ret;
    }
    break;
