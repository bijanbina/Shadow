...
static int server_handshake_reply(int udp_assc, struct socks5_response *response)
{
...
static int server_handshake(buffer_t *buf)
{
    server_ctx_t *server_recv_ctx = NULL;// (server_ctx_t *)w;
    server_t *server              = server_recv_ctx->server;
    remote_t *remote              = server->remote;
    ...
    if (!remote->direct)
    {
        int err = crypto->stream_encrypt(abuf, server->e_ctx, SOCKET_BUF_SIZE);
        if (err)
        {
...
static server_t * new_server(int fd)
{
    ...
    server->e_ctx = new ScStream(sc_password);
    server->d_ctx = new ScStream(sc_password);
